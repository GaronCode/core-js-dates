...
function getNextFriday(date) {
  return new Date(
    Number(date) + (((11 - date.getUTCDay()) % 7) + 1) * 86_400_000
  );
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (new Date(dateEnd) - new Date(dateStart)) / 86_400_000 + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return (
    conditionDate >= new Date(period.start) &&
    conditionDate <= new Date(period.end)
  );
}
...
function getWeekNumberByDate(date) {
  ...
  dataDay.setDate((firstDay <= 4 ? 2 : 9) - firstDay);
  return Math.floor((date.getTime() - dataDay.getTime()) / 604_800_000 + 1);
}
/**
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const toDate = (dateStr) => {
    ...
    return new Date(`${dateSplited[2]}-${dateSplited[1]}-${dateSplited[0]}`);
  };
  const workingDates = [];
  ...
  for (
    ...
    if (workDays < countWorkDays) {
      ...
      workDays += 1;
    } else if (offDays + 1 < countOffDays) offDays += 1;
    else {
      offDays = 0;
      workDays = 0;
    }
